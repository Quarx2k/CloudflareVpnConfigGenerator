...
public class GooglePlayAPI {
	 ...
	 * 
	 */
	public AndroidCheckinResponse checkin(boolean loginAC2DM) throws Exception {
		...
		AndroidCheckinRequest.Builder builder = checkInbuilder
				.setId(new BigInteger(this.getAndroidID(), 16).longValue())
				.setSecurityToken(new BigInteger(getSecurityToken(), 16).longValue());

		AndroidCheckinRequest build;
		if (loginAC2DM) {
			build = builder.addAccountCookie("[" + getEmail() + "]").addAccountCookie(c2dmAuth).build();
		} else {
			build = builder.build();
		}
	 ...
	 */
	public BuyResponse purchase(String packageName, int versionCode,
			int offerType) throws IOException {
